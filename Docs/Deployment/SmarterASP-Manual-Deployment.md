...
The Release A production prerequisite for Release B is satisfied. Release B is implemented locally, not production verified. It uses standard application HSTS middleware only in Production for `myvocabularybuilder.org` over HTTPS, before Swagger/static files/authentication. Its intentional five-minute initial policy is `Strict-Transport-Security: max-age=300`, without `includeSubDomains` or `preload`. Development HTTP remains usable. Review, Git/CI gates, production approval, deployment and live HSTS verification remain pending. Keep HTTPS available during recovery: removing middleware does not clear browser policy cached for up to 300 seconds after the last header. See the [production completion record](../Updates/PSH-1-https-ssl-production-hardening-implementation-plan.md#18-release-a-production-completion); its operational and recovery caveats remain applicable.
...
Use the current validated PSH-1 artifact procedure; do not manually reconstruct or edit a production package to bypass its safeguards:
...
6. Require post-MSDeploy transport acceptance and manual authenticated HTTPS Angular/API smoke. Record acceptance or follow the documented manual recovery procedure on failure. Neither release requires a database migration. Release A is production accepted; Release B adds the mandatory 300-second HSTS acceptance checks and still requires its own production approval and verification.
