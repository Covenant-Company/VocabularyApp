...
- **HTTPS (PSH-1 Release A)**: The published root IIS configuration redirects HTTP navigation to the canonical HTTPS site and rejects insecure API/unsafe requests with 403. Implementation is pending production deployment and acceptance; do not assume the currently deployed site enforces it. Keep SmarterASP.NET 1-Click Force HTTPS disabled. HSTS is deferred to Release B. See the [Release A implementation record](Updates/PSH-1-https-ssl-production-hardening-implementation-plan.md#17-release-a-implementation-record).
